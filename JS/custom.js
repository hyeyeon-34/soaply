// 프로그래밍 언어 조합 약속 :
// menu_icon : 스네이크 케이스
// menu-icon : 하이픈 케이스(오류 나는 경우가 많아서 css에서 많이 적어줌)
// menuIcon : 카멜케이스

// addEventListener(): 이벤트 리스너 추가
// contains() : 클래스 포함 여부 - boolean
// getAttribute() : 속성값 가져오기 - 파라미터가 1개 (파라미터로 지정된 속성의 값을 가져온다)
// setAtrribute() : 속성값 설정하기 - 파라미터가 2개 ( 첫번째 파라미터는 설정할 속성 이름, 두번째 파라미터는 설정할 속성의 값)
...
setTimeout(() => {
  const swiper = new Swiper('.swiper', {
    // Optional parameters
    // direction: 'vertical', // default가 가로(horizontal)
    loop: true, // 슬라이더 무한반복
